...
Caveats retained: PageUp and government routes remain under-researched; public-looking ATS endpoints require tenant/terms review; source-policy evidence can age; `jobspy-js` may not honor AbortSignal despite caller composition; parser network isolation requires deployment enforcement. Historical Python JobSpy sidecar and deleted legacy RAG JobSpy paths are retained here only as historical evidence, not current runtime claims.
