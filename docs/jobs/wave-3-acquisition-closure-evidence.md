...
| W3-I semantic_jobs shadow (historical; deleted) | deleted source/test (historical audit only)                                                                                  | superseded                             | —       |
| W3-J closure evidence (this doc)                | `test/jobs/w3jAcquisitionClosure.test.ts`                                                                                    | —                                      | —       |

W3-H additive exports are registered in `src/jobs/acquisition/index.ts` with smoke checks in `test/jobs/acquisitionExports.test.ts`. W3-I source/test were deleted as superseded historical comparison artifacts; no live path imports or exports them.
...
| historical command: `npm test -- test/jobs/semanticJobsShadow.test.ts` (deleted test; audit result only)                                                                                                                                                                                                                                                                                               | historical pass — not current-state evidence     |
...
- Historical W3-I was a read-only shadow comparison. Its source/test were deleted; this retained description records migration audit history only and does not describe a live path, cutover, dual-write, or flag flip.
...
- W3-H imports no persistence, graph DB, SQLite, corpus cache, logger, or `query_log` modules; this is enforced by its import-boundary test and the coordinator import-allowlist test. Historical W3-I import-boundary evidence is retained only in the recorded audit result; its source/test are deleted.
