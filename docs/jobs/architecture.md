...
**Status:** Authoritative design. Wave 0, Wave 1, Wave 3, and bounded non-persistent Wave 2A authorized. Wave 2B, production persistence, and unresolved retention/encryption gates remain blocked. See [indexed-discovery amendment](indexed-discovery-amendment.md).
...
`jobs.sqlite` holds migrations, postings, listings, observations, memberships, evidence, claims/resolutions, requirements, locations, identity decisions/clusters, snapshots/diffs, lifecycle events, runs/slices/candidates/results, enrichment cache, source health, and policy revisions. `profiles.sqlite` is pending encryption ADR and must remain separate.
