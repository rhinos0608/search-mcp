...
| `JOBSPY_BOARDS`        | All boards (temporary) | Comma-separated JobSpy board names for `jobs_search`/`jobs` (e.g. `linkedin,indeed`). Unset = all supported boards enabled (temporary local default; public release restores opt-in: unset = JobSpy disabled). Supported names: `linkedin`, `indeed`, `zip_recruiter`, `glassdoor`, `google`, `google_careers`, `bayt`, `naukri`, `bdjobs`. Unknown names are dropped with a warning; max 16. Overrides `jobsAcquisition.jobspyBoards` in config. |
