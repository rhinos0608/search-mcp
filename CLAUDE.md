...
- **Family tools** (9): `src/tools/families/`, registered via `registerFamily()`. Discriminated-union `action` field per family.
- **Standalone tools** (5): `src/tools/standalone/`, direct `server.registerTool()`.
...
See AGENTS.md for full action lists. Config-gated tools: `web_crawl`, `semantic_crawl*`, `browser`. Jobs acquisition is policy-gated in-process.

**Standalone**: web_search, rss, web_crawl, jobs_search, health_check.

**Families**: github (9 actions), youtube (3), reddit (3), research (15), packages (2), browser (24), agentic_browse (4), semantic_crawl (3), jobs (3).

## RAG Pipeline (`src/rag/`)

Shared by semantic_crawl, youtube.semantic, reddit.semantic, github.code_search.
...
Key: `pipeline.ts`, `embedding.ts`, `profiles.ts`, `bm25.ts`, `fusion.ts`, `dedup.ts`, `corpusCache.ts`, `rerank.ts`, `adapters/` (text, transcript, conversation, code, academic, qa), `code/` (tree-sitter, symbol extraction).
...
- Adapter types: `code`, `text`, `transcript`, `conversation`, `academic`, `qa`
