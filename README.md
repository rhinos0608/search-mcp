...
| Category         | Tools                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Web**          | `web_search` (Codex main source; all configured Exa/Brave/SearXNG/DuckDuckGo/Tavily/Jina/Firecrawl/Diffbot/Ollama backends fan out in parallel — key-gated, **billable** per-provider, see cost warning; URL-dedupe keeping richest representation; bare-Markdown output with per-block `[N-M]` citations and 192 KiB budget; excerpt-only by default; `aiSummary` no/yes/only for Exa/Tavily native summaries; optional semantic rerank with source-credibility floor; auto-enrich thin document snippets via in-process PDF/Office parsing), `rss` (RSS/Atom parse/search/monitor), `web_crawl` ⚙️, `agentic_browse` family (`browse`/`present`/`read`/`focus`) |
| **Semantic RAG** | `semantic_crawl` ⚙️ (crawl/list_corpora/inspect_corpus actions over URL/sitemap/search/github/cached sources, BM25+embedding+RRF)                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| **Jobs**         | `jobs_search` + `jobs` family — policy-gated in-process acquisition, structured extraction, constraint filtering, and deterministic ranking                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------- |
| `SEARCH_BACKEND`               | Fallback preference: `brave`, `searxng`, `exa`, `duckduckgo`, `ollama-search`, `tavily`, `jina`, `firecrawl`, `diffbot`, or `codex`; all configured providers run |
...
| `JINA_API_KEY`                 | Jina Search API key (`s.jina.ai`) — **billable**; see warning below                                                               |
| `FIRECRAWL_API_KEY`            | Firecrawl v2 search API key — **billable**; see warning below                                                                     |
| `FIRECRAWL_SCRAPE_FALLBACK_ENABLED` | `true`/`false` (default `false`). Second gate for the billable web_crawl Firecrawl scrape fallback; requires the key          |
| `DIFFBOT_API_KEY`              | Diffbot Web Search API key (`llm.diffbot.com`) — **billable**; see warning below                                                  |
...
> **⚠️ PAID-PROVIDER COST WARNING — Jina, Firecrawl, Diffbot.** Setting `JINA_API_KEY`, `FIRECRAWL_API_KEY`, or `DIFFBOT_API_KEY` (via env or `config.json`) opts that provider into the all-provider fanout — there is no separate search enable flag. Because `web_search` fans out to **every configured backend for every query variation** (query expansion can multiply requests 2–3×), configuring a key means **every search query issues billable requests to that vendor**: Jina ≈10,000+ tokens per search, Firecrawl 2 credits per 10 results, Diffbot credits per query. Costs multiply with usage — a single interactive search can hit several vendors at once. Health checks deliberately make NO live calls to these providers (config-only status); the dashboard provider test also makes no paid call. Excluding a provider costs nothing: leave its key unset. `FIRECRAWL_SCRAPE_FALLBACK_ENABLED=true` additionally allows `web_crawl` to fall back to a billable Firecrawl scrape of the seed URL when Crawl4AI fails (Crawl4AI remains the required primary; the fallback never runs without `CRAWL4AI_BASE_URL` and never bypasses request guards; multi-page crawls degrade to the seed page with an explicit warning).
...
1. **Parallel fanout:** All configured backends (Codex, Exa, Brave, SearXNG, DuckDuckGo, Tavily, Jina, Firecrawl, Diffbot, Ollama) execute simultaneously. `SEARCH_BACKEND` controls fallback ordering only — it does not limit provider scope. Jina/Firecrawl/Diffbot join via their API keys and are billable per request (see cost warning above).
