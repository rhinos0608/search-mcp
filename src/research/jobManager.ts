...
export interface ResearchJobSummary {
  ...
  updatedAt: number;
  completedAt: number | undefined;
  /** Path to the persisted result file on disk, if saved. */
  resultFile: string | undefined;
}
...
export class ResearchJobManager {
   ...
   * Optionally clears the full result from memory (lazy load via poll/getResult later).
   */
  setResultFile(jobId: string, filePath: string, offload = false): void {
    const job = this.jobs.get(jobId);
    if (!job) return;
  ...
  /**
   * Manually register a job (used for re-hydrating saved jobs from disk).
   */
  registerJob(snapshot: ResearchJobSnapshot): void {
    if (this.jobs.has(snapshot.jobId)) return;

    const job: InternalJob = {
      ...snapshot,
      // Ensure result is offloaded if from disk (poll will re-load)
      result: undefined,
      abortController: undefined,
      runtimeTimeout: undefined,
    };

    this.jobs.set(snapshot.jobId, job);
  }
  ...
  private runCleanup(): void {
    ...
    for (const [id, job] of this.jobs) {
      ...
      if (terminalTime !== undefined && now - terminalTime > this.ttlMs) {
        // "Jobs that are saved should be saved forever... jobs that don't get saved get a 24 hour cache."
        // If saved, we just offload the result (if not already done) but keep the job entry.
        // If not saved, we transition to expired.
        if (job.resultFile) {
          if (job.result) {
            job.result = undefined;
            job.updatedAt = now;
            logger.debug({ jobId: id }, 'Offloaded saved research result from memory after TTL');
          }
          continue;
        }
        ...
        job.status = 'expired';
        job.updatedAt = now;
        logger.debug({ jobId: id }, 'Research job expired (not saved)');
        continue;
      }
