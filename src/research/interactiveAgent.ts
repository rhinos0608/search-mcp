...
export class InteractiveBrowserAgent {
  ...
  async executePlan(
    ...
    try {
            ...
            case 'wait': {
              if (action.value !== undefined) {
                ...
                if (Number.isFinite(ms) && ms >= 0) {
                  ...
                  logger.warn(
                    { value: action.value },
                    'InteractiveAgent: invalid wait value, skipping',
                  );
                }
            ...
            case 'scroll': {
              if (action.value !== undefined) {
                ...
                if (Number.isFinite(pixels)) {
                  ...
                  logger.warn(
                    { value: action.value },
                    'InteractiveAgent: invalid scroll value, skipping',
                  );
                }
              }
              break;
      ...
      warnings.push(
        `Interactive extraction failed: ${err instanceof Error ? err.message : String(err)}`,
      );
      return {
        content: '',
        title: '',
      ...
      if (ownSession && this.session) {
        await browserManager.close(this.session).catch(() => {
          /* ignore close errors */
        });
        this.session = null;
      }
