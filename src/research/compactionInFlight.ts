...
export class InFlightCompactor {
  ...
  compact(): CompactionReport {
    ...
    const stateSizeBefore =
      snapshot.sources.length + snapshot.findings.length + snapshot.gaps.length;
    ...
    const sizeAfter2 =
      snapshotAfter2.sources.length + snapshotAfter2.findings.length + snapshotAfter2.gaps.length;
    ...
    const sizeAfter3 =
      snapshotAfter3.sources.length + snapshotAfter3.findings.length + snapshotAfter3.gaps.length;
  ...
  private compressFindings(): number {
    ...
    logger.info({ modified }, 'InFlight compaction stage 3: compressed finding excerpts');
  ...
  private claimCheckState(): { evicted: number; path: string | null } {
    ...
    try {
      ...
      logger.info(
        {
          filePath,
          stateSize: snapshot.sources.length + snapshot.findings.length + snapshot.gaps.length,
        },
        'InFlight compaction stage 4: wrote full state to claim-check file',
      );
    } catch (err) {
