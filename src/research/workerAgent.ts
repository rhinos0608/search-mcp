...
export class WorkerAgent {
  ...
  async investigate(
    ...
    if (this.config.deterministicMode) {
      ...
      logger.info(
        { question: truncate(question, 80), queries: plan.queries.length },
        'Worker planning (LLM)',
      );
    }
    ...
    if (this.config.maxSubThreadDepth > 0 && allSubThreads.length > 0) {
      const topThreads = allSubThreads.filter((t) => t.priority <= 2).slice(0, 2);
    ...
    try {
      ...
      if (result.response.content && result.response.content.length > 20) {
        logger.warn(
          ...
          'Worker planning returned non-JSON text; using fallback',
        );
        return {
          queries: [question],
          sourceTypes: ['web', 'academic', 'reddit', 'youtube', 'hackernews'],
  ...
  private async executeSearches(plan: { queries: string[]; sourceTypes: string[] }): Promise<{
    urls: {
      title: string;
      url: string;
      snippet: string;
    ...
    for (const query of plan.queries) {
      ...
      if (plan.sourceTypes.includes('web') || plan.sourceTypes.length === 0) {
        searchPromises.push(
          getBreaker('web')
            .execute(() => this.tools.webSearch(query, 10))
            .then((r) => {
              addResults(
                r.map((x) => ({
                  title: x.title,
                  ...
                  ...(x.extraSnippet ? { extraSnippet: x.extraSnippet } : {}),
                  ...(x.deepLinks ? { deepLinks: x.deepLinks } : {}),
                })),
                'web',
              );
            })
            .catch((err: unknown) => {
              if (err instanceof CircuitBreakerOpenError) {
                logger.debug({ backend: 'web' }, 'Circuit breaker open, skipping web search');
              }
            }),
        );
      }
      ...
      if (plan.sourceTypes.includes('academic')) {
        searchPromises.push(
          getBreaker('academic')
            .execute(() => this.tools.academicSearch(query, 5))
            .then((r) => {
              addResults(
                r.map((x) => ({ ...x, description: x.abstract ?? '' })),
                'academic',
              );
            })
            .catch((err: unknown) => {
              if (err instanceof CircuitBreakerOpenError) {
                logger.debug(
                  { backend: 'academic' },
                  'Circuit breaker open, skipping academic search',
                );
              }
            }),
        );
      ...
      if (plan.sourceTypes.includes('github')) {
        searchPromises.push(
          getBreaker('github')
            .execute(() => this.tools.githubSearch(query, 5))
            .then((r) => {
              addResults(
                r.map((x) => ({ title: x.fullName, url: x.htmlUrl, description: x.description })),
                'github',
              );
            })
            .catch((err: unknown) => {
              if (err instanceof CircuitBreakerOpenError) {
                logger.debug({ backend: 'github' }, 'Circuit breaker open, skipping github search');
              }
            }),
        );
      }
    ...
    if (primaryQuery) {
      ...
      searchPromises.push(
        getBreaker('reddit')
          .execute(() => this.tools.redditSearch(primaryQuery, 5))
          .then((r) => {
            addResults(
              r.map((x) => ({ ...x, description: x.selftext ?? '' })),
              'reddit',
            );
          })
          .catch((err: unknown) => {
            if (err instanceof CircuitBreakerOpenError) {
              logger.debug({ backend: 'reddit' }, 'Circuit breaker open, skipping reddit search');
            }
          }),
      );
      // Hacker News
      searchPromises.push(
        getBreaker('hackernews')
          .execute(() => this.tools.hackernewsSearch(primaryQuery, 5))
          .then((r) => {
            addResults(
              r.map((x) => ({ ...x, description: x.text ?? '' })),
              'hackernews',
            );
          })
          .catch((err: unknown) => {
            if (err instanceof CircuitBreakerOpenError) {
              logger.debug(
                { backend: 'hackernews' },
                'Circuit breaker open, skipping hackernews search',
              );
            }
          }),
      );
      ...
      searchPromises.push(
        getBreaker('youtube')
          .execute(() => this.tools.semanticYoutube(primaryQuery, { maxVideos: 5, topK: 5 }))
          .then((r) => {
          ...
          .catch((err: unknown) => {
            if (err instanceof CircuitBreakerOpenError) {
              logger.debug({ backend: 'youtube' }, 'Circuit breaker open, skipping youtube search');
            }
          }),
      );
      // Reddit semantic — deeper community context
      searchPromises.push(
        getBreaker('reddit-semantic')
          .execute(() => this.tools.semanticReddit(primaryQuery, { maxPosts: 5, topK: 5 }))
          .then((r) => {
            for (const chunk of r.chunks) {
              this.semanticContentCache.set(chunk.url, chunk.text);
            }
          ...
          .catch((err: unknown) => {
            if (err instanceof CircuitBreakerOpenError) {
              logger.debug(
                { backend: 'reddit-semantic' },
                'Circuit breaker open, skipping reddit semantic search',
              );
            }
          }),
      );
      ...
      searchPromises.push(
        getBreaker('wikipedia')
          .execute(() => this.tools.wikipediaSearch(primaryQuery))
          .then((r) => {
            addResults(
              r.map((x) => ({ title: x.title, url: x.link, description: x.snippet })),
              'wikipedia',
            );
          })
          .catch((err: unknown) => {
            if (err instanceof CircuitBreakerOpenError) {
              logger.debug(
                { backend: 'wikipedia' },
                'Circuit breaker open, skipping wikipedia search',
              );
            }
          }),
      );
      ...
      searchPromises.push(
        getBreaker('pubmed')
          .execute(() => this.tools.pubmedSearch(primaryQuery, 5))
          .then((r) => {
            addResults(
              r.map((x) => ({ title: x.title, url: x.link, description: x.snippet })),
              'pubmed',
          ...
          .catch((err: unknown) => {
            if (err instanceof CircuitBreakerOpenError) {
              logger.debug({ backend: 'pubmed' }, 'Circuit breaker open, skipping pubmed search');
            }
          }),
      );
      ...
      searchPromises.push(
        getBreaker('stackoverflow')
          .execute(() => this.tools.stackoverflowSearch(primaryQuery, 5))
          .then((r) => {
            addResults(
              r.map((x) => ({ title: x.title, url: x.link, description: x.bodySnippet })),
              'stackoverflow',
            );
          })
          .catch((err: unknown) => {
            if (err instanceof CircuitBreakerOpenError) {
              logger.debug(
                ...
                'Circuit breaker open, skipping stackoverflow search',
              );
            }
          }),
      );
  ...
  private selectTopPages(
    ...
    for (const u of urls) {
      ...
      existing.push({
        ...
        ...(u.extraSnippet ? { extraSnippet: u.extraSnippet } : {}),
        ...(u.deepLinks ? { deepLinks: u.deepLinks } : {}),
      });
      ...
      if (totalChars + blockLen > maxChars) {
        ...
        if (block.score > 0.3 && selected.length < 3) {
          const truncated = truncate(
            block.text,
            Math.min(block.text.length, maxChars - totalChars),
          );
  ...
  private computeRelevance(text: string, heading: string, questionTerms: Set<string>): number {
    ...
    for (const term of questionTerms) {
      ...
      // Check text body
      const count = (
        lowerText.match(new RegExp(term.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) ?? []
      ).length;
      if (count > 0) {
    ...
    const normalizedScore =
      (score / Math.max(1, questionTerms.size * 3)) * 0.6 + matchRatio * 0.3 + lengthNorm * 0.1;
  ...
  private async synthesizeFromCompacted(
    ...
    if (compacted.length === 0) {
      logger.info(
        { question: truncate(question, 60) },
        'WorkerAgent: no compacted content to synthesize',
      );
    ...
    if (this.config.deterministicMode || !this.llm) {
      ...
      logger.info(
        { question: truncate(question, 60) },
        'WorkerAgent: deterministic synthesis (regex)',
      );
    ...
    contextParts.push(
      `--- Compacted source content (${String(compacted.length)} sections from ${String(uniqueSources.size)} sources: ${[...new Set(uniqueSources.values())].join(', ')}) ---`,
    );
    contextParts.push('');
...
Output ONLY valid JSON:
{
  ...
  "subThreads": [
    ...
    try {
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a research synthesis analyst. Read compacted source material and produce structured findings. Output ONLY valid JSON.',
          },
      ...
      if (result.response.content && result.response.content.length > 200) {
        logger.warn(
          { question: truncate(question, 60) },
          'Worker synthesis returned text instead of JSON; wrapping as finding',
        );
      ...
      throw new Error(
        `Worker synthesis failed: ${result.response.error ?? 'Unknown error'}. Guidance required.`,
      );
    } catch (err) {
  ...
  private async chaseSubThread(
    ...
    const searchPromises: Promise<
      { title: string; url: string; snippet: string; sourceType: string; extraSnippet?: string }[]
    >[] = [];

    if (thread.suggestedSourceTypes.includes('web') || thread.suggestedSourceTypes.length === 0) {
      searchPromises.push(
        this.tools
          .webSearch(thread.question, 5)
          .then((r) =>
            r.map((x) => ({
              title: x.title,
              url: x.url,
    ...
    if (thread.suggestedSourceTypes.includes('academic')) {
      searchPromises.push(
        this.tools
          .academicSearch(thread.question, 3)
          .then((r) =>
            r.map((x) => ({
              title: x.title,
              url: x.url,
    ...
    searchPromises.push(
      this.tools
        .redditSearch(thread.question, 3)
        .then((r) =>
          r.map((x) => ({
            title: x.title,
            url: x.url,
    ...
    searchPromises.push(
      this.tools
        .hackernewsSearch(thread.question, 3)
        .then((r) =>
          r.map((x) => ({
            title: x.title,
            url: x.url,
  ...
  private extractFindingsRegex(compacted: CompactBlock[]): WorkerFinding[] {
    ...
    const patterns = [
      {
        name: 'mechanism',
        regex: /\b(uses?|utilizes?|employs?|leverages?|implements?)\s+([^.]{10,100})/i,
      },
  ...
  private buildNarrativeSummary(
    ...
    if (promotionalSources > 0) {
      parts.push(
        `(${String(promotionalSources)} promotional source(s) were examined but down-weighted)`,
      );
    }
