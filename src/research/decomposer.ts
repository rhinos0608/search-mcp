  ...
  'current-events': [
    {
      ...
      failureModes: [
        'no major announcements found',
        'outdated information',
        'vendor-driven announcements without independent verification',
      ],
      budgetPriority: 1,
    },
    {
      textTemplate:
        'What concrete capability improvements, features, or performance gains have been demonstrated in {topic}?',
      evidenceType: 'empirical',
    ...
    {
      textTemplate:
        'What benchmarks, evaluation results, or performance metrics have been reported for {topic}?',
      evidenceType: 'benchmark',
      preferredSources: ['web', 'news', 'academic', 'github'],
      freshnessRequirement: 'within 6 months',
      failureModes: [
        'benchmarks not standardized',
    ...
    {
      textTemplate:
        'How have industry experts, practitioners, and the community responded to developments in {topic}?',
      evidenceType: 'practitioner',
      preferredSources: ['reddit', 'hackernews', 'web', 'news'],
      freshnessRequirement: 'within 3 months',
      failureModes: [
        'echo chamber effects',
        'small sample size in community reactions',
        'selection bias',
      ],
      budgetPriority: 4,
    },
    {
      textTemplate:
        'What are the broader implications, adoption trends, and expected next developments for {topic}?',
      evidenceType: 'trend',
      preferredSources: ['news', 'web', 'academic', 'hackernews'],
      freshnessRequirement: 'within 6 months',
      failureModes: [
        'speculative predictions not marked as such',
        'hype-driven analysis',
...
const KNOWN_ENTITIES: {
  ...
  // Fusion energy
  {
    ...
    queryContext: 'ITER international tokamak experimental reactor project status and timeline',
  },
  {
    pattern: /\bSPARC\b/i,
    name: 'SPARC',
    domain: 'fusion energy',
    queryContext:
      'SPARC compact fusion reactor by Commonwealth Fusion Systems progress and milestones',
  },
  {
    pattern: /\bJET\b/i,
    name: 'JET',
    domain: 'fusion energy',
    queryContext: 'JET Joint European Torus fusion experiment results and decommissioning',
    contextTerms: ['fusion', 'tokamak', 'plasma', 'iter', 'nuclear'],
  },
  {
    pattern: /\bNIF\b/i,
    name: 'NIF',
    domain: 'fusion energy',
    queryContext: 'NIF National Ignition Facility inertial confinement fusion breakthroughs',
  },
  {
    pattern: /\b(tokamak|stellarator)\b/i,
    name: 'tokamak/stellarator',
    domain: 'fusion energy',
    queryContext:
      'tokamak and stellarator magnetic confinement fusion reactor designs latest advances',
  },
  // Quantum computing
  {
    pattern: /\bIBM\s+Quantum\b/i,
    name: 'IBM Quantum',
    domain: 'quantum computing',
    queryContext: 'IBM Quantum computing roadmap and processor milestones',
  },
  {
    pattern: /\bSycamore\b/i,
    name: 'Sycamore',
    domain: 'quantum computing',
    queryContext: 'Google Sycamore quantum processor quantum supremacy experiment',
  },
  // Space
  {
    pattern: /\b(JWST|James Webb)\b/i,
    name: 'JWST',
    domain: 'space telescopes',
    queryContext: 'James Webb Space Telescope latest discoveries and observations',
  },
  {
    pattern: /\bArtemis\b/i,
    name: 'Artemis',
    domain: 'space exploration',
    queryContext: 'NASA Artemis program Moon mission timeline and progress',
    contextTerms: ['nasa', 'moon', 'lunar', 'orion', 'sls'],
  },
  {
    pattern: /\bStarship\b/i,
    name: 'Starship',
    domain: 'space exploration',
    queryContext: 'SpaceX Starship development test flights and milestones',
    contextTerms: ['spacex', 'rocket', 'launch', 'mars', 'elon'],
  },
  // AI/ML
  {
    pattern: /\bGPT-?4\b/i,
    name: 'GPT-4',
    domain: 'AI models',
    queryContext: 'GPT-4 capabilities benchmarks and technical specifications',
  },
  {
    pattern: /\bClaude\b/i,
    name: 'Claude',
    domain: 'AI models',
    queryContext: 'Anthropic Claude AI model capabilities and releases',
    contextTerms: ['ai', 'anthropic', 'llm', 'model', 'gpt'],
  },
  {
    pattern: /\bGemini\b/i,
    name: 'Gemini',
    domain: 'AI models',
    queryContext: 'Google Gemini multimodal AI model capabilities and benchmarks',
    contextTerms: ['ai', 'google', 'llm', 'model', 'gpt'],
  },
  // Climate/Energy
  {
    pattern: /\bCOP\d{2}\b/i,
    name: 'COP',
    domain: 'climate policy',
    queryContext: 'UN climate conference outcomes and agreements',
  },
  {
    pattern: /\bIPCC\b/i,
    name: 'IPCC',
    domain: 'climate science',
    queryContext: 'IPCC assessment report findings and climate projections',
  },
];
  ...
  for (const entity of KNOWN_ENTITIES) {
    ...
    if (match) {
      ...
      if (entity.contextTerms && entity.contextTerms.length > 0) {
        const hasContext = entity.contextTerms.some((term) =>
  ...
  for (const { entity } of matches) {
    if (!seen.has(entity.name)) {
      ...
      entities.push({
        ...
        queryContext: entity.queryContext,
      });
    }
  }
...
const INSTITUTION_QUERY_PATTERN =
  /\b(scientology|church|religion|cult|organization|organisation|institution|movement|nonprofit|non-profit|ngo|company|corporation|political party|agency|group)\b/i;

const GENERIC_INSTITUTION_FACETS: CoverageFacet[] = [
  {
    textTemplate:
      'What do independent sources say about the membership, active participation, scale, and claimed-versus-verified reach of {topic}?',
    evidenceType: 'scale-verification',
    preferredSources: ['academic', 'web', 'news', 'wikipedia'],
    freshnessRequirement: 'within 5 years',
    failureModes: ['only self-reported scale claims found', 'no independent estimates'],
    budgetPriority: 1,
  },
  {
    textTemplate:
      'What are the finances, revenue model, assets, real-estate holdings, and opaque funding structures connected to {topic}?',
    evidenceType: 'financial-scale',
    preferredSources: ['news', 'web', 'academic'],
    freshnessRequirement: 'within 5 years',
    failureModes: ['financial claims unsourced', 'assets described only by advocates or critics'],
    budgetPriority: 2,
  },
  {
    textTemplate:
      'What legal, regulatory, civil-process, and current litigation issues define the public record around {topic}?',
    evidenceType: 'legal-record',
    preferredSources: ['web', 'news', 'academic'],
    freshnessRequirement: 'within 2 years',
    failureModes: ['legal tangent drift', 'missing primary court or regulator documents'],
    budgetPriority: 1,
  },
  {
    textTemplate:
      'What front groups, affiliates, cover names, or adjacent programs are used by {topic}, and how do independent sources connect them?',
    evidenceType: 'affiliate-map',
    preferredSources: ['news', 'web', 'academic'],
    freshnessRequirement: 'within 5 years',
    failureModes: [
      'affiliate claims not connected to primary evidence',
      'misses public-facing cover names',
    ],
    budgetPriority: 2,
  },
];

const TOPIC_SPECIFIC_FACETS: CoverageFacet[] = [
  {
    pattern: /\bscientology\b/i,
    textTemplate:
      'What do independent estimates say about Scientology membership versus official Church claims, including active global membership estimates?',
    evidenceType: 'membership-discrepancy',
    preferredSources: ['academic', 'news', 'web'],
    freshnessRequirement: 'within 10 years',
    failureModes: ['official membership claims repeated without independent estimates'],
    budgetPriority: 1,
  },
  {
    pattern: /\bscientology\b/i,
    textTemplate:
      'What is the Rehabilitation Project Force (RPF), what do defectors and courts allege about detention or labour conditions, and how has the Church responded?',
    evidenceType: 'internal-practice',
    preferredSources: ['news', 'academic', 'web'],
    freshnessRequirement: 'any',
    failureModes: ['treats RPF as a footnote', 'no defector testimony or legal record'],
    budgetPriority: 1,
  },
  {
    pattern: /\bscientology\b/i,
    textTemplate:
      'What happened in the Lisa McPherson case, including the death, criminal charges, civil litigation, settlement, and organizational impact?',
    evidenceType: 'case-study',
    preferredSources: ['news', 'web', 'academic'],
    freshnessRequirement: 'any',
    failureModes: ['missing one of death, charges, settlement, or aftermath'],
    budgetPriority: 1,
  },
  {
    pattern: /\bscientology\b/i,
    textTemplate:
      'How do Narconon and other Scientology-linked front groups or social programs operate under separate names?',
    evidenceType: 'front-groups',
    preferredSources: ['news', 'web', 'academic'],
    freshnessRequirement: 'within 10 years',
    failureModes: ['does not connect cover names to Scientology', 'ignores Narconon'],
    budgetPriority: 2,
  },
  {
    pattern: /\bscientology\b/i,
    textTemplate:
      'What is known about David Miscavige and current civil-process or litigation evasion allegations?',
    evidenceType: 'current-litigation',
    preferredSources: ['news', 'web'],
    freshnessRequirement: 'within 2 years',
    failureModes: ['stale leadership coverage', 'missing civil process context'],
    budgetPriority: 2,
  },
  {
    pattern: /\bscientology\b/i,
    textTemplate:
      'What did the 1990s alt.religion.scientology internet conflict reveal about institutional attempts to suppress online criticism?',
    evidenceType: 'internet-history',
    preferredSources: ['web', 'academic', 'wikipedia'],
    freshnessRequirement: 'any',
    failureModes: ['finds article but misses the suppression story'],
    budgetPriority: 3,
  },
  {
    pattern: /\bscientology\b/i,
    textTemplate:
      'What primary documents and reporting explain the 1993 IRS settlement and tax-exemption agreement with Scientology?',
    evidenceType: 'primary-documents',
    preferredSources: ['web', 'news', 'academic'],
    freshnessRequirement: 'any',
    failureModes: ['no primary IRS or agreement documents', 'only tertiary summary'],
    budgetPriority: 1,
  },
];

function generateCoverageFacetSubQuestions(query: string, topic: string): SubQuestion[] {
  const facets: CoverageFacet[] = [];
  if (INSTITUTION_QUERY_PATTERN.test(query)) {
    facets.push(...GENERIC_INSTITUTION_FACETS);
  }
  facets.push(...TOPIC_SPECIFIC_FACETS.filter((facet) => facet.pattern?.test(query)));

  const pendingStatus: SubQuestionStatus = 'pending';
  return facets.map((facet) => ({
    id: makeId(),
    text: fillTemplate(facet.textTemplate, topic),
    classification: 'explainer',
    evidenceType: facet.evidenceType,
    preferredSources: [...facet.preferredSources],
    freshnessRequirement: facet.freshnessRequirement,
    failureModes: [...facet.failureModes],
    budgetPriority: facet.budgetPriority,
    status: pendingStatus,
  }));
}

function dedupeSubQuestions(subQuestions: SubQuestion[]): SubQuestion[] {
  const seen = new Set<string>();
  const deduped: SubQuestion[] = [];
...
export class QueryDecomposer {
  ...
  decompose(query: string): DecomposeResult {
    ...
    subQuestions.push(...generateCoverageFacetSubQuestions(query, topic));
    ...
    if (dedupedSubQuestions.length < minSQs) {
      logger.warn(
        ...
        `Decomposition produced ${String(dedupedSubQuestions.length)} sub-questions for a ${classification} query — below minimum of ${String(minSQs)}. Consider query expansion.`,
      );
    }
  ...
  async llmDecompose(
    ...
    subQuestions.push(...generateCoverageFacetSubQuestions(query, disambiguatedQuery));
    const dedupedSubQuestions = dedupeSubQuestions(subQuestions);
    ...
    if (dedupedSubQuestions.length < minSQs) {
      logger.warn(
        { classification, subQuestionCount: dedupedSubQuestions.length, minRequired: minSQs },
        `LLM decomposition produced only ${String(dedupedSubQuestions.length)} sub-questions for a ${classification} query — below minimum of ${String(minSQs)}. SubQuestionCount of 0 or very low suggests the agent is not properly decomposing.`,
      );
    }
    ...
    const plan = `${classification.charAt(0).toUpperCase() + classification.slice(1)} research on "${query}". LLM-decomposed into ${String(dedupedSubQuestions.length)} sub-questions.`;
