 ...
 *   2. Trim findings: top-K by confidence, drop noise, truncate verbosity
...
function compactSingleFinding(f: Finding, maxExcerptChars: number): CompactFinding {
  ...
  return {
    ...
    corroboratingSourceCount: f.corroboratingSourceIds.length,
    contradictingSourceCount: f.contradictingSourceIds.length,
...
/**
 * Sort findings by confidence descending, drop noise, cap at maxFindings.
 *
 * Rules:
 *   - Drop findings with confidence < minConfidence
 *   - Keep at most maxFindings entries
 *   - Return flat array sorted globally by confidence
 */
export function trimFindings(
  allFindings: Finding[],
  maxFindings: number,
  options: Required<CompactionOptions>,
): { compact: CompactFinding[]; droppedLow: number; droppedCap: number } {
  const { minConfidence, maxExcerptChars } = options;
  // Strategy: sort all findings by confidence desc, apply filters globally.

  const sorted = [...allFindings].sort((a, b) => b.confidence - a.confidence);

  let droppedLow = 0;
  const aboveThreshold: Finding[] = [];

  for (const f of sorted) {
    if (f.confidence < minConfidence) {
      droppedLow++;
    } else {
      aboveThreshold.push(f);
    }
  }

  // Cap at maxFindings, but at least keep top 5
  const cap = Math.max(5, maxFindings);
  const capped = aboveThreshold.slice(0, cap);
  const droppedCap = aboveThreshold.length - capped.length;

  const compact = capped.map((f) => compactSingleFinding(f, maxExcerptChars));

  return { compact, droppedLow, droppedCap };
}
 ...
 * Truncates executive summary first, then drops lowest-confidence findings.
 */
function applySizeGuard(
  ...
  // Still over hard limit — drop findings from the bottom (lowest confidence)
  const findings = [...compact.findings].sort((a, b) => b.confidence - a.confidence);
  let dropped = 0;
 ...
 *   - Top findings by confidence (trimmed)
 *   - Summary statistics
...
export function compactResearchResult(
  ...
  const {
    ...
    droppedLow: dl,
    droppedCap: dc,
  } = trimFindings(allFindings, maxFindings, opts);
  ...
  // Build compact findings deduplicated globally, sorted by confidence
