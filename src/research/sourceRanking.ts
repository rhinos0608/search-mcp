...
export function rankSource(
  ...
  const readPriorityScore = Math.min(1, (
    freshness * 0.3 +
    source.sourceConfidencePrior * 0.2 +
    depthPenalty * 0.2 +
    (source.isPrimary ? 0.3 : 0)
  ) * freqBoost);
  ...
  const evidenceWeight = Math.min(1, (
    authority * 0.4 +
    typeBase * 0.3 +
    source.sourceConfidencePrior * 0.2 +
    (source.isPrimary ? 0.1 : 0)
  ));
...
function sourceTypeBaseWeight(type: SourceType): number {
  switch (type) {
    ...
    case 'github': return 0.8;
    case 'documentation': return 0.8;
    case 'news': return 0.6;
    case 'stackoverflow': return 0.6;
    case 'web': return 0.5;
    case 'reddit': return 0.4;
    case 'hackernews': return 0.4;
    case 'youtube': return 0.4;
    case 'podcast': return 0.4;
    case 'producthunt': return 0.4;
    case 'patent': return 0.7;
    default: return 0.5;
  }
}
