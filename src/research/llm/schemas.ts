...
const evidenceDirectnessEnum = z.enum(['direct', 'near-direct', 'secondary', 'anecdotal', 'speculative']);
const claimTypeEnum = z.enum(['primary', 'secondary', 'anecdotal']);
const sourceTypeEnum = z.enum(['academic', 'web', 'github', 'reddit', 'hackernews', 'stackoverflow', 'documentation', 'news', 'patent', 'podcast', 'producthunt', 'youtube', 'browser-interactive']);
...
const contradictionStatusEnum = z.enum(['unresolved', 'partially_resolved', 'resolved', 'apparent_only']);
...
export const EvaluateResultSchema = z.object({
  ...
  missingDimensions: z.array(z.string()).min(0).describe('Dimensions, sub-questions, or perspectives not adequately explored')
});
...
export const DecideResultSchema = z.object({
  ...
  subQuestionIds: z.array(z.string()).optional().describe('Sub-question IDs this action should focus on')
});
...
export const ExtractResultSchema = z.object({
  findings: z.array(z.object({
    claim: z.string().min(10).describe('Verbatim or near-verbatim claim as stated in the source text'),
    evidenceExcerpt: z.string().min(10).describe('Direct quote (1-3 sentences) supporting the claim'),
    evidenceDirectness: evidenceDirectnessEnum.describe('Evidence directness level'),
    claimType: claimTypeEnum.describe('Claim type')
  })).min(0).describe('Extracted findings')
});
...
export const AuditResultSchema = z.object({
  ...
  issues: z.array(z.object({
    ...
    description: z.string().min(20).describe('Detailed, specific description with IDs and quotes'),
  ...
  })).min(0).describe('List of issues found'),
  stats: z.object({
    ...
    sourceDiversity: z.array(z.object({
      type: sourceTypeEnum,
...
export const TreeGenerateQueriesResultSchema = z.object({
  ...
  })).min(1).describe('Generated search queries')
});
...
export const TreeProcessResultsResultSchema = z.object({
  ...
  })).min(0).describe('Extracted learnings'),
...
export const ResearchReportSchema = z.object({
  ...
  executiveSummary: z.string().min(100).describe('2-4 paragraphs of flowing prose with inline [N] citations'),
  themes: z.array(z.object({
    title: z.string().min(5).describe('Theme name'),
    narrative: z.string().min(100).describe('2-4 paragraphs of flowing prose with inline citation markers')
  })).min(1).describe('Thematic analysis themes'),
  contradictions: z.array(z.object({
    ...
    likelyExplanation: z.string().optional().describe('Explanation if resolved or apparent only')
  })).min(0).describe('Contradictions and debates'),
...
export const ResearchReportV2Schema = z.object({
  ...
  executiveSummary: z.string().min(100).describe('2-4 paragraphs of flowing prose with [Source N] citations'),
  keyFindings: z.array(z.object({
    ...
    }).optional().describe('Evidence quality assessment'),
    citations: z.array(z.string()).min(0).describe('Inline [Source N] citation markers used')
  })).min(1).describe('Key findings grouped by theme'),
  contradictionsDebates: z.array(z.object({
  ...
  sourceQualityAssessment: z.object({
    diversity: z.object({
    ...
    promotionalContentDetected: z.boolean().optional().describe('Whether promotional content was detected'),
    qualityConcerns: z.array(z.string()).optional().describe('Systematic quality concerns')
  }).describe('Source quality assessment'),
  uncertaintiesLimitations: z.array(z.object({
  ...
  })).min(0).describe('Uncertainties and limitations'),
  openQuestions: z.array(z.string()).min(0).describe('Legitimate questions remaining'),
  recommendations: z.string().optional().describe('Actionable recommendations if decision-oriented'),
  metadata: z.object({
