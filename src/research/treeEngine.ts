...
export class DeepTreeResearchEngine {
  ...
  async run(
    ...
    await this.onProgress(
      35,
      `Starting tree research: breadth=${String(breadth)}, depth=${String(depth)}`,
    );
    ...
    for (const r of settled) {
      if (r.status === 'fulfilled') {
        ...
        logger.warn(
          ...
          'Tree research: query processing failed',
        );
      }
    }
    ...
    try {
      const result = await this.llm.callJSON<{ queries: TreeQuery[] }>({
        ...
        messages: [
          ...
          {
            role: 'user',
            content: `Generate ${String(numQueries)} search queries to research: ${query}`,
          },
        ],
        temperature: 0.7,
      });
