...
export class PruningEngine {
   ...
   tierFindings(
      findings: Finding[],
   ): { confirmed: Finding[]; corroborated: Finding[]; unverified: Finding[] } {
   ...
   private deduplicateFindings(findings: Finding[]): Finding[] {
         ...
         for (let j = i + 1; j < findings.length; j++) {
            ...
            if (sim > 0.85) {
               ...
               if (existing) {
                  ...
                  existing.subQuestionIds = [...new Set([...existing.subQuestionIds, ...fj.subQuestionIds])];
               } else {
      ...
      for (const f of findings) {
         ...
         if (merge) {
            result.push({ ...f, sourceIds: merge.sourceIds, subQuestionIds: merge.subQuestionIds, lastUpdated: new Date().toISOString() });
         } else {
            result.push(f);
         }
      }
