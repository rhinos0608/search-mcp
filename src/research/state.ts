...
/** Overlap coefficient over content-word sets. Strong for near-verbatim restatements. */
...
/** Combined similarity: max of Jaccard, trigram, and containment overlap. */
function combinedSimilarity(a: string, b: string): number {
  ...
  return Math.max(jaccard, trigram, containment);
}
...
export class ResearchStateEngine {
  ...
  /**
   * Deduplicate findings by normalized claim similarity.
   *
   * Repeated research workers often restate the same policy definition or event
   * with minor wording changes. Use an aggressive lexical containment check for
   * high-overlap restatements, while requiring either strong similarity or shared
   * provenance for borderline pairs to avoid collapsing genuinely distinct facts.
   */
  private deduplicateFindings(): number {
    ...
    for (let i = 0; i < findings.length; i++) {
      const fi = findings[i];
      if (!fi || absorbed.has(fi.id)) continue;

      for (let j = i + 1; j < findings.length; j++) {
        const fj = findings[j];
        if (!fj || absorbed.has(fj.id)) continue;

        const sim = combinedSimilarity(fi.normalizedClaim, fj.normalizedClaim);
        const sharedSubQuestion = fi.subQuestionIds.some((id) => fj.subQuestionIds.includes(id));
        const sharedSource = fi.sourceIds.some((id) => fj.sourceIds.includes(id));
        const duplicate = sim >= 0.72 || (sim >= 0.56 && (sharedSubQuestion || sharedSource));
