...
export function computeGates(params: GateComputationParams): Gate[] {
   ...
   const noActiveGaps = state.gaps.filter(
   ...
   if (lastEvaluation && !lastEvaluation.pass) {
      ...
      const hasNewEvidence =
         activeGap !== undefined &&
         (extractedDelta > 0 || sourceStats.pending > 0);
      if (!hasNewEvidence) {
         answerAllowed = false;
   ...
   const gates: Gate[] = [
      { action: 'answer', allowed: answerAllowed, ...(answerReason !== undefined ? { reason: answerReason } : {}) },
      { action: 'decompose', allowed: decomposeAllowed, ...(decomposeReason !== undefined ? { reason: decomposeReason } : {}) },
      { action: 'discover', allowed: discoverAllowed, ...(discoverReason !== undefined ? { reason: discoverReason } : {}) },
      { action: 'extract', allowed: extractAllowed, ...(extractReason !== undefined ? { reason: extractReason } : {}) },
      { action: 'fill_gaps', allowed: fillGapsAllowed, ...(fillGapsReason !== undefined ? { reason: fillGapsReason } : {}) },
      { action: 'generate_queries', allowed: generateQueriesAllowed, ...(generateQueriesReason !== undefined ? { reason: generateQueriesReason } : {}) },
      { action: 'contradiction_scan', allowed: contradictionScanAllowed },
      { action: 'audit', allowed: true },
      { action: 'synthesize', allowed: true, ...(synthesizeReason !== undefined ? { reason: synthesizeReason } : {}) },
