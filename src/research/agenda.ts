...
export class Agenda {
  ...
  enqueue(
    target: Omit<GapTarget, 'id' | 'normalizedQuestion' | 'status' | 'attempts' | 'createdAtStep'>,
  ): GapTarget | undefined {
    ...
    for (const existing of this.targets) {
      ...
      if (this.options.enableSemanticDedup) {
        ...
        if (
          sim >= this.options.jaccardThreshold &&
          (existing.status === 'open' || existing.status === 'active')
        ) {
          return undefined;
        }
      }
    ...
    const newTarget: GapTarget = {
      ...
      ...(target.parentId !== undefined
        ? { parentId: target.parentId, parentQuestion: target.parentQuestion }
        : {}),
      status: 'open',
