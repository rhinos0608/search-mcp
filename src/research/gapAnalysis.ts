...
export class GapAnalyzer {
  ...
  private thinCoverage(coverage: SubQuestionCoverage[]): GapRecord[] {
    ...
    for (const sq of coverage) {
      ...
      const category = sq.uniqueDomainCount <= 1 ? 'single_source_dependency' : 'thin_coverage';
  ...
  private promotionalBias(
    ...
    for (const sq of coverage) {
      ...
      const sqSources = state.sources.filter((s) => s.subQuestionId === sq.subQuestionId);
...
export class GapFiller {
  ...
  shouldContinueLoop(): boolean {
    ...
    // 3. Minimum sanity passes: for standard/deep runs, do at least a small
    // number of gap-loop passes before low-priority and coverage heuristics
    // are allowed to declare the topic complete.
    if (currentLoops < minGapLoops) return true;

    // 4. All open gaps are low priority?
    if (openGaps.every((g) => g.priority > 3)) return false;

    // 5. Check if most sub-questions are well-covered (>3 sources AND >2 source types)
    ...
    const wellCoveredRatio =
      state.subQuestions.length > 0 ? wellCoveredCount / state.subQuestions.length : 0;
