...
export function getLastConsolidationTime(): number | null {
  ...
  try {
    const row = db
      .prepare(
        'SELECT created_at FROM kg_projection_checkpoints WHERE projection_version = 0 AND id = ?',
      )
      .get('__consolidation_last_run') as { created_at: string } | undefined;
...
function saveConsolidationTime(): void {
  ...
  try {
    db.prepare(
      'INSERT OR REPLACE INTO kg_projection_checkpoints (projection_version, id, created_at) VALUES (0, ?, ?)',
    ).run('__consolidation_last_run', new Date().toISOString());
  } catch {
    /* best effort */
  }
}
...
function buildMergePrompt(
  familyA: { label: string; description: string | null },
  familyB: { label: string; description: string | null },
): string {
  return [
    `Family A: ${familyA.label}`,
    familyA.description ? `  Description: ${familyA.description}` : '  (no description)',
    '',
    `Family B: ${familyB.label}`,
    familyB.description ? `  Description: ${familyB.description}` : '  (no description)',
    '',
    'Should these families be merged?',
  ].join('\n');
}
...
function parseMergeResponse(content: string): { shouldMerge: boolean; reason?: string } {
  ...
  if (trimmed.startsWith('YES')) {
    const reason = content.trim().slice(3).trim() || 'LLM judged merge appropriate';
...
export async function runConsolidationPass(
  ...
  try {
    ...
    if (families.length >= config.consolidation.annThreshold) {
      logger.warn(
        { familyCount: families.length },
        'kg: consolidation skipped — exceeds ANN threshold, V7.1+ required',
      );
      return result;
    }
    ...
    for (const fam of families) {
      try {
        ...
        if (db !== null) {
          const row = db
            .prepare('SELECT embedding FROM kg_embeddings WHERE object_id = ? AND object_type = ?')
            .get(fam.id, 'family') as { embedding: Buffer } | undefined;
          if (row?.embedding) {
            const blob = row.embedding;
            const vec: number[] = [];
            for (let i = 0; i < blob.byteLength / 4; i++) vec.push(blob.readFloatLE(i * 4));
            embedding = vec;
          }
        }
        if (embedding === null && cfgGlobal.embeddingSidecar.baseUrl) {
          const response = await embedTexts({
            texts: [fam.description ? `${fam.label}: ${fam.description}` : fam.label],
            mode: 'document',
            dimensions,
          });
          ...
          if (embedding)
            storeEmbedding(
              fam.id,
              'family',
              response.model || cfgGlobal.embeddingSidecar.provider,
              embedding,
              fam.id,
            );
        }
        ...
        logger.warn({ err, familyId: fam.id }, 'kg: embed failed');
      }
    }
    ...
    for (let i = 0; i < familyEmbeddings.length && !capReached; i++) {
      for (let j = i + 1; j < familyEmbeddings.length; j++) {
        ...
        const familyA = getFamily(a.id),
          familyB = getFamily(b.id);
        ...
        const llmResponse = await callSimpleLlm(
          cfgGlobal.llm,
          MERGE_SYSTEM_PROMPT,
          buildMergePrompt(familyA, familyB),
        );
        if (!llmResponse.success) {
          ...
          if (db) {
            try {
              db.prepare(
                'INSERT OR REPLACE INTO kg_family_merge_candidates (family_a, family_b, confidence, reason, generated_at) VALUES (?, ?, ?, ?, ?)',
              ).run(
                a.id,
                b.id,
                sim,
                `LLM unavailable, sim=${sim.toFixed(3)}`,
        ...
        if (parsed.shouldMerge && sim >= AUTO_MERGE_CONFIDENCE) {
          appendEvents([
            {
              ...
              payload: JSON.stringify({
                ...
                into_id: b.id,
                confidence: sim,
                reason: parsed.reason ?? 'Auto-merge',
              }),
              payloadHash: null,
            },
          ]);
          ...
          try {
            db.prepare(
              'INSERT OR REPLACE INTO kg_family_merge_candidates (family_a, family_b, confidence, reason, generated_at, consolidation_version) VALUES (?, ?, ?, ?, ?, ?)',
            ).run(
              a.id,
              b.id,
              sim,
              parsed.shouldMerge
                ? `LLM: ${parsed.reason ?? 'merge'}, sim=${sim.toFixed(3)}`
                : `LLM: ${parsed.reason ?? 'keep separate'}, sim=${sim.toFixed(3)}`,
              new Date().toISOString(),
              'v7.0',
            );
            result.candidatesCreated++;
    ...
    logger.warn({ err }, 'kg: runConsolidationPass failed');
    result.durationMs = Date.now() - startedAt;
    return result;
  }
}
