...
export interface Pass2RelationResult {
  newRelations: number;
  removedRelations: number;
  familyPairsChecked: number;
}
...
function getLastPass2RunId(): string | null {
  ...
  try {
    const row = db
      .prepare('SELECT ref_id FROM kg_event_refs WHERE event_id = ? AND ref_type = ?')
      .get(LAST_PASS_2_KEY, 'checkpoint') as { ref_id: string } | undefined;
...
function setLastPass2RunId(): void {
  ...
  try {
    ...
    const lastEvent = db.prepare('SELECT id FROM kg_events ORDER BY id DESC LIMIT 1').get() as
      | { id: string }
      | undefined;
    const refId = lastEvent?.id ?? String(Date.now());
    db.prepare(
      'INSERT OR REPLACE INTO kg_event_refs (event_id, ref_type, ref_id) VALUES (?, ?, ?)',
    ).run(LAST_PASS_2_KEY, 'checkpoint', refId);
  } catch {
    /* best effort */
  }
}
...
export function shouldRunPass2(): boolean {
  try {
    ...
    const runCountRow = db
      .prepare(
        "SELECT COUNT(*) as cnt FROM kg_events WHERE event_type = ? AND timestamp > (SELECT COALESCE(MAX(timestamp), '') FROM kg_events WHERE id = ?)",
      )
      .get('RUN_COMPLETED', lastRunId) as { cnt: number } | undefined;
    return (runCountRow?.cnt ?? 0) >= 5;
  } catch (err) {
    logger.warn({ err }, 'kg: shouldRunPass2 failed');
    return false;
  }
}
...
function buildRelationPrompt(
  ...
  return [
    `Family A: ${familyA.label}`,
    familyA.description ? `  Description: ${familyA.description}` : '',
    '',
    `Family B: ${familyB.label}`,
    familyB.description ? `  Description: ${familyB.description}` : '',
    '',
    `Shared entities (${String(sharedEntityLabels.length)}): ${sharedEntityLabels.join(', ')}`,
    sharedEdgeTypes.length > 0 ? `Cross-family edges: ${sharedEdgeTypes.join(', ')}` : '',
    '',
    'What is the relationship, if any? Respond with RELATION <type> <reason> or NONE.',
  ]
    .filter(Boolean)
    .join('\n');
}

function parseRelationResponse(content: string): {
  relationType: FamilyRelationType | null;
  reason?: string;
} {
  ...
  if (match?.[1])
    return {
      relationType: match[1].toLowerCase() as FamilyRelationType,
...
async function checkTemporalSupersedes(familyA: KgFamily, familyB: KgFamily): Promise<boolean> {
  try {
    ...
    const aSource = db
      .prepare(
        'SELECT MAX(retrieved_at) as latest FROM kg_sources WHERE run_id IN (SELECT first_seen_run_id FROM kg_nodes WHERE id IN (SELECT node_id FROM kg_node_families WHERE family_id = ?))',
      )
      .get(familyA.id) as { latest: string | null } | undefined;
    const bSource = db
      .prepare(
        'SELECT MAX(retrieved_at) as latest FROM kg_sources WHERE run_id IN (SELECT first_seen_run_id FROM kg_nodes WHERE id IN (SELECT node_id FROM kg_node_families WHERE family_id = ?))',
      )
      .get(familyB.id) as { latest: string | null } | undefined;
    const aTime = aSource?.latest
      ? new Date(aSource.latest).getTime()
      : familyA.lastActivity
        ? new Date(familyA.lastActivity).getTime()
        : 0;
    const bTime = bSource?.latest
      ? new Date(bSource.latest).getTime()
      : familyB.lastActivity
        ? new Date(familyB.lastActivity).getTime()
        : 0;
    return aTime > bTime;
  } catch {
    return false;
...
async function detectStaleRelations(families: KgFamily[]): Promise<number> {
  ...
  try {
        ...
        appendEvents([
          {
            ...
            eventType: 'FAMILY_RELATION_REMOVED',
            eventVersion: 1,
            runId: ev.runId,
            batchId: null,
            actor: 'classifier',
            ...
            payload: JSON.stringify({
              ...
              family_a: familyA,
              family_b: familyB,
              reason: 'One or both families no longer exist',
            }),
            payloadHash: null,
          },
        ]);
    ...
    logger.warn({ err }, 'kg: detectStaleRelations failed');
    return removed;
  }
}
...
export async function runPass2Relations(): Promise<Pass2RelationResult> {
  ...
  const result: Pass2RelationResult = {
    newRelations: 0,
    removedRelations: 0,
    familyPairsChecked: 0,
  };
  try {
    const cfgGlobal = loadConfig();
    const kgConfig = cfgGlobal.knowledgeGraph;
    if (!cfgGlobal.llm.baseUrl || !cfgGlobal.llm.provider) {
      logger.info('kg: LLM not configured, skipping Pass 2');
      return result;
    }
    ...
    for (const fam of families) {
      familyNodeIds.set(
        fam.id,
        queryNodes({ familyId: fam.id, limit: maxNodesPerFamily }).nodes.map((n) => n.id),
      );
    }
    ...
    for (const [famId, nids] of familyNodeIds) {
      for (const nid of nids) {
        ...
        if (!families) {
          families = new Set();
          nodeToFamilies.set(nid, families);
        }
        families.add(famId);
      }
    }
    ...
    for (const pair of pairsToCheck) {
      try {
        ...
        const aNodeIds = familyNodeIds.get(pair.familyA.id) ?? [],
          bNodeIds = new Set(familyNodeIds.get(pair.familyB.id) ?? []);
        const sharedEntityLabels: string[] = [],
          sharedEdgeTypes: string[] = [];
        for (const nid of aNodeIds) {
          if (bNodeIds.has(nid)) {
            const node = getNode(nid);
            if (node !== null) sharedEntityLabels.push(node.label);
            for (const edge of getEdgesForNode(nid)) {
              if (bNodeIds.has(edge.fromId) || bNodeIds.has(edge.toId)) {
                if (!sharedEdgeTypes.includes(edge.type)) sharedEdgeTypes.push(edge.type);
              }
            }
          }
        }
        const llmResponse = await callSimpleLlm(
          ...
          buildRelationPrompt(
            pair.familyA,
            pair.familyB,
            sharedEntityLabels.length > 0 ? sharedEntityLabels : ['(shared via edges)'],
            sharedEdgeTypes,
          ),
        );
        if (!llmResponse.success) {
          logger.warn({ error: llmResponse.error }, 'kg: Pass2 LLM failed');
          continue;
        }
        const parsed = parseRelationResponse(llmResponse.content);
        if (parsed.relationType === null) continue;
        if (
          parsed.relationType === 'supersedes' &&
          !(await checkTemporalSupersedes(pair.familyA, pair.familyB))
        ) {
          logger.info(
            { familyA: pair.familyA.id, familyB: pair.familyB.id },
            'kg: supersedes skipped — no temporal evidence',
          );
          continue;
        }
        ...
        appendEvents([
          {
            ...
            batchId: null,
            actor: 'classifier',
            entityId: pair.familyA.id,
            entityType: 'family',
            payload: JSON.stringify({
              relation_id: relationId,
              family_a: pair.familyA.id,
              family_b: pair.familyB.id,
        ...
        logger.info(
          {
            familyA: pair.familyA.label,
            familyB: pair.familyB.label,
        ...
        logger.warn(
          { err, familyA: pair.familyA.id, familyB: pair.familyB.id },
          'kg: Pass2 failed for pair',
        );
      }
    }
    ...
    logger.warn({ err }, 'kg: runPass2Relations failed');
    return result;
  }
}
