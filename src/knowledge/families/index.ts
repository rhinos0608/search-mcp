...
export async function runFamilyPipelineAfterRun(
  runEntities: RunEntity[],
  runMetadata: RunMetadata,
  runId: string,
  config: KnowledgeGraphConfig,
): Promise<FamilyPipelineResult> {
  logger.info({ runId, entityCount: runEntities.length }, 'kg: runFamilyPipelineAfterRun starting');
  const result: FamilyPipelineResult = {
    classifier: { assignmentCount: 0, newCandidateCount: 0 },
    solidified: 0,
    relations: null,
    consolidation: null,
  };
  ...
  try {
    ...
    result.classifier = {
      assignmentCount: classifierResult.assignments.length,
      newCandidateCount: classifierResult.newCandidates.length,
    };
    logger.info(
      {
        assignmentCount: result.classifier.assignmentCount,
        newCandidateCount: result.classifier.newCandidateCount,
      },
      'kg: Pass 1 complete',
    );
  } catch (err) {
    logger.warn({ err }, 'kg: Pass 1 failed, continuing');
  }
  ...
  try {
    ...
    logger.warn({ err }, 'kg: solidification failed, continuing');
  }
  ...
  try {
    if (shouldRunPass2()) {
      ...
      logger.info(
        { newRelations: result.relations.newRelations, removed: result.relations.removedRelations },
        'kg: Pass 2 complete',
      );
    } else {
      logger.info('kg: Pass 2 skipped');
    }
  } catch (err) {
    logger.warn({ err }, 'kg: Pass 2 failed, continuing');
  }
  ...
  try {
    ...
    if (shouldRunConsolidation(lastConsolidation, config)) {
      ...
      logger.info(
        {
          pairsChecked: result.consolidation.pairsChecked,
          autoMerged: result.consolidation.autoMerged,
          candidatesCreated: result.consolidation.candidatesCreated,
        },
        'kg: consolidation complete',
      );
    } else {
      logger.info('kg: consolidation skipped');
    }
  } catch (err) {
    logger.warn({ err }, 'kg: consolidation failed, continuing');
  }
