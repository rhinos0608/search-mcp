...
import { fetchFocus } from './tools/fetchFocus.js';
      ...
      try {
        ...
        if (isDocumentUrl(url) && cfg.raga.enabled && cfg.raga.baseUrl) {
          try {
            ...
            const { markdown, warnings: ragaWarnings } = await extractWithRAGA(
              url,
        ...
        try {
          ...
          if (isDocumentUrl(url) && cfg.raga.enabled && cfg.raga.baseUrl) {
            try {
              ...
              warnings.push(
                `RAGA extraction failed: ${String(ragaErr)}. Falling back to Crawl4AI.`,
              );
            }
          }
