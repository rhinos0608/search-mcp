...
export function checkExtractionQuality(
	...
	if (textLength < cfg.minTextLength) {
		issues.push(
			`Text too short: ${textLength} chars (min: ${cfg.minTextLength})`,
		);
	...
	const hasBody = result.elements?.some(
	...
	const tableCount =
		result.elements?.filter((e) => e.type === "table").length || 0;
	if (tableCount > 3) {
		issues.push(
			`Many tables detected (${tableCount}), may need structured extraction`,
		);
	...
	const imageCount =
		result.elements?.filter((e) => e.type === "image").length || 0;
	if (imageCount > 10) {
		issues.push(
			`Many images detected (${imageCount}), may need specialized processing`,
		);
...
export function shouldEscalateToRAGA(
	...
	if (contentType) {
		...
		if (documentTypes.some((dt) => normalizedType.includes(dt))) {
			logger.debug(
				{ contentType },
				"Escalating to RAG-Anything: document type",
			);
			return true;
		}
