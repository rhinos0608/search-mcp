...
export async function getOrBuildCorpus(
  ...
  // Variant suffix distinguishes differently-built corpora for the same source
  // (e.g. with vs. without contextual embeddings). Empty string == no variant.
  const variantSuffix = variant ? `|${variant}` : '';

  // Stable key for pendingBuilds map — includes variant to prevent cross-talk
  const sourceKey = stableStringify(normalizeSource(source)) + variantSuffix;
  ...
  const promise = (async (): Promise<CachedCorpus> => {
    ...
    const corpusId = computeCorpusId(source, model, dimensions, variant);
    const now = Date.now();
