...
const PLATFORM_DETECTORS: PlatformDetector[] = [
  {
    ...
    docPatterns: [/github\.com\/[^/]+\/[^/]+/, /__NEXT_DATA__/],
  },
  {
    platform: 'gitlab',
    priority: 10,
    linePatterns: [/^repository$/i, /^merge requests/i, /^ci\/cd/i, /^deploy tokens/i],
  ...
  },
  {
    ...
    priority: 10,
    linePatterns: [],
    docPatterns: [/cloudflare/i, /checking if the site connection is secure/i, /ray id:/i],
  },
  {
    platform: 'medium',
    priority: 5,
...
function isNavLikeLine(trimmed: string): boolean {
  ...
  if (
    /^\s*[*\-+]\s+(?:platform|solutions|resources|products|company|explore|learn|more from|tools|developer|partners|support|enterprise|team|pricing|blog|docs|help|status|community|about|careers|press|contact|terms|privacy|cookies|by\s+\w+)/i.test(
      trimmed,
    )
  )
    return true;
...
function computeStructuralSignals(
  ...
  const shortLines = nonEmpty.filter((l) => {
    ...
    return (
      trimmed.length > 0 &&
      trimmed.length < SHORT_LINE_THRESHOLD &&
      !trimmed.startsWith('```') &&
      !trimmed.startsWith('~~~')
    );
  });
  ...
  const authLines = nonEmpty.filter((l) => {
    const trimmed = l.trim();
    return BOILERPLATE_FAMILIES.find((f) => f.family === 'auth_permission')?.patterns.some((p) =>
      p.test(trimmed),
    );
  });
  ...
  const consentLines = nonEmpty.filter((l) => {
    const trimmed = l.trim();
    return BOILERPLATE_FAMILIES.find((f) => f.family === 'consent_compliance')?.patterns.some((p) =>
      p.test(trimmed),
    );
  });
  ...
  const jsShellLines = nonEmpty.filter((l) => {
    const trimmed = l.trim();
    return BOILERPLATE_FAMILIES.find((f) => f.family === 'spa_js_shell')?.patterns.some((p) =>
      p.test(trimmed),
    );
  });
  ...
  const errorLines = nonEmpty.filter((l) => {
    const trimmed = l.trim();
    return BOILERPLATE_FAMILIES.find((f) => f.family === 'error_placeholder')?.patterns.some((p) =>
      p.test(trimmed),
    );
  });
  ...
  // Bullet links (single)
  const bulletLinkCount = nonEmpty.filter((l) =>
    /^\s*[*\-+]\s+\[[^\]]+\]\([^)]+\)\s*$/.test(l.trim()),
  ).length;
...
function computePositiveScore(signals: ContentSignals): {
  score: number;
  ...
  const raw: QualitySignal[] = [
    ...
    {
      name: 'deepHeadingsWithBody',
      value: Math.min(signals.deepHeadingWithBodyCount / 4, 1),
      weight: 15,
    },
...
function computeNegativeScore(structural: StructuralSignals): {
  score: number;
  signals: QualitySignal[];
} {
  const raw: QualitySignal[] = [
...
function classifyQuality(
  ...
  const extremeHeadingNoBody =
    structural.headingWithoutBodyRatio > 0.8 && contentSignals.contentWordCount < 50;
  ...
  if (
    extremeConsent ||
    (families.includes('consent_compliance') && structural.consentLineCount >= 3)
  ) {
  ...
  if (normalized.length < MIN_MEANINGFUL_CHARS && positiveScore < 20) {
    reasons.push(
      `too little content (${String(normalized.length)} chars, ${String(contentSignals.contentWordCount)} words)`,
    );
  ...
  if (!hasDecentContent && negativeScore > 40) {
    reasons.push(
      `mixed low-quality content (positive=${String(Math.round(positiveScore))}, negative=${String(Math.round(negativeScore))})`,
    );
...
function computeRecoveryRecommendation(
  classification: QualityClassification,
...
export function assessMarkdownBatchQuality(
  ...
  if (meaningfulAssessments.length > 0) {
    ...
    const sorted = [...meaningfulAssessments].sort((a, b) => b.score.positive - a.score.positive);
    const best = sorted[0];
  ...
  const worst = assessments.reduce<MarkdownQualityAssessment>(
    (min, a) => (a.score.overall < min.score.overall ? a : min),
    first,
  );
...
export function compareQuality(
  ...
  if (beforePos.length > 0 && afterPos.length > 0) {
    ...
    if (bHeadings && aHeadings)
      deltas.deepHeadingWithBodyCount = Math.round((aHeadings.value - bHeadings.value) * 4);
  ...
  if (beforeNeg.length > 0 && afterNeg.length > 0) {
    ...
    if (bBoiler && aBoiler)
      deltas.boilerplateLineRatio = Math.round((aBoiler.value - bBoiler.value) * 100);
