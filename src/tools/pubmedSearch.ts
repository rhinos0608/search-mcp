...
export async function searchPubMed(query: string, limit = 10): Promise<PubMedResult[]> {
   ...
   try {
      ...
      esearchJson = JSON.parse(esearchText);
   } catch (err) {
      logger.warn({ err, query }, 'PubMed ESearch failed');
      return [];
   }

   const idlist = (esearchJson as any)?.esearchresult?.idlist;
   if (!Array.isArray(idlist) || idlist.length === 0) {
      return [];
   }

   // ── Phase 2: EFetch — get abstracts ──────────────────────────────────
   const ids = (idlist as string[]).slice(0, effectiveLimit).join(',');
   ...
   try {
      ...
      return (idlist as string[]).slice(0, effectiveLimit).map((id) => ({
         title: `PubMed ID: ${id}`,
