...
const familyListSchema = z.object({
  ...
  limit: z.number().int().min(1).max(200).optional().default(50).describe('Max families to return (default 50)'),
});
        ...
        if (db !== null && result.families.length > 0) {
          ...
          try {
            const nodeCountRows = db
              .prepare(`SELECT family_id, COUNT(*) as cnt FROM kg_node_families WHERE family_id IN (${placeholders}) GROUP BY family_id`)
            ...
            const mergeRows = db
              .prepare(`SELECT family_a, family_b, reason, confidence FROM kg_family_merge_candidates WHERE family_a IN (${placeholders})`)
              .all(...familyIds) as { family_a: string; family_b: string; reason: string | null; confidence: number | null }[];
