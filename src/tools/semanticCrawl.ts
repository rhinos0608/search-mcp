      ...
      async (seedUrl: string) => {
         ...
         if (isDocumentUrl(seedUrl) && ragaConfig?.enabled && ragaConfig.baseUrl) {
            try {
               ...
               logger.warn({ seedUrl }, 'semantic_crawl: RAGA extraction failed, falling back to Crawl4AI');
            }
         }
...
export async function semanticCrawl(
   ...
   switch (opts.source.type) {
      ...
      case 'sitemap': {
         ...
         const result = await crawlSeeds(safeUrls, crawl4aiCfg, sitemapOpts, ragaConfig);
         crawlWarnings.push(...result.warnings);
         for (const sw of result.structuredWarnings) {
            crawlStructuredWarnings.push(sw);
         }
