...
const AU_CITIES = new Set([
  ...
  'townsville', 'cairns', 'toowoomba', 'paramatta', 'penrith',
]);
...
const US_CITIES = new Set([
  ...
  'tampa', 'tulsa', 'arlington', 'wichita', 'cleveland',
  'bakersfield', 'aurora', 'anaheim', 'honolulu', 'santa ana',
  'riverside', 'corpus christi', 'lexington', 'stockton', 'henderson',
  'st. louis', 'st paul', 'cincinnati', 'pittsburgh', 'greensboro',
  'ann arbor', 'annapolis', 'burlington', 'richmond', 'longmont',
  'asheville', 'west des moines', 'tysons corner',
]);
  ...
  if (location.length > 0) {
    ...
    for (const [name, code] of Object.entries(COUNTRY_NAMES)) {
      if (location.includes(name)) {
        ...
        reasons.push(`Location text mentions "${name}" → ${positive ? 'positive' : 'negative'} signal`);
        break;
      }
    }
