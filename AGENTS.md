...
Config-gated: `web_crawl` (Crawl4AI), `semantic_crawl*` (Crawl4AI + embedding), `semantic_jobs` (embedding + search), `deep_research` (`DEEP_RESEARCH_ENABLED`), `browser` (`BROWSER_ENABLED`), `knowledge_graph` (`KG_ENABLED`). Family actions may also be individually gated.
...
```bash
# Search (at least one)
EXA_API_KEY, BRAVE_API_KEY, SEARXNG_BASE_URL, SEARCH_BACKEND (brave|searxng|exa|tavily), TAVILY_API_KEY
# Social
