...
test('github.file accepts repository URL field', () => {
  ...
  const parsed = (entry.inputSchema as { safeParse: (v: unknown) => { success: boolean; data?: unknown } }).safeParse({
    action: 'file',
    repository: 'https://github.com/owner/repo',
    path: 'README.md',
  });
...
test('github.code_search accepts GitHub URL in repo field (transformed at per-action validate)', () => {
  ...
  const result = (entry.inputSchema as { safeParse: (v: unknown) => { success: boolean } }).safeParse({
    action: 'code_search',
    query: 'function foo',
