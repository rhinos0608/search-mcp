...
async function fetchCorpus(opts?: { fileFilter?: string[] }): Promise<GitHubCorpusDocument[]> {
  // Real corpus applies fileFilter to tree entries before downloading content.
  // Mock simulates this by filtering DOCS paths.
  const docs = DOCS;
  if (!opts?.fileFilter || opts.fileFilter.length === 0) return docs;
  return docs.filter((doc) =>
    opts.fileFilter!.some((f) => {
      if (f.length === 0) return true;
      if (f.includes('*')) {
        const pat = f.replace(/[.+?^${}()|[\]\\]/gu, '\\$&').replace(/\*/gu, '.*');
        return new RegExp(`^${pat}$`, 'u').test(doc.path);
      }
      return doc.path.startsWith(f) || doc.path.includes(f);
    }),
  );
}
...
test('semanticGitHubCode returns controlled warning for empty corpora', async () => {
  ...
  assert.ok(
    result.warnings.some(
      (warning) => /No files found in/i.test(warning) || /No GitHub files matched/i.test(warning),
    ),
    `Expected empty-corpora warning, got: ${JSON.stringify(result.warnings)}`,
  );
});
...
test('semanticGitHubCode forwards scopePath and extensions to corpus', async () => {
  let observedOptions: any;
  await semanticGitHubCode(
    {
      query: 'formatName',
  ...
  assert.equal(observedOptions?.scopePath, 'src');
  assert.deepEqual(observedOptions?.extensions, ['.ts', '.tsx']);
  assert.deepEqual(observedOptions?.excludeExtensions, ['.test.ts']);
});
