...
test('searchWikidata respects limit parameter', async () => {
  const results = await searchWikidata('test', 'en', 2);
  assert.ok(results.length <= 2, `expected <= 2 results, got ${String(results.length)}`);
});

test('searchWikidata handles empty query gracefully', async () => {
  const results = await searchWikidata('', 'en', 5);
  assert.ok(Array.isArray(results), 'should return an array');
});
