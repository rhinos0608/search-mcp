...
import { buildAliasPattern, type ProjectContext } from '../../src/research/projectContext.js';
...
// ── Test project context (simulates researching MCP) ────────────────────────

const mcpCtx: ProjectContext = {
  canonicalName: 'Model Context Protocol',
  aliases: ['mcp', 'model context protocol'],
  projectTextPattern: buildAliasPattern(['mcp', 'model context protocol']),

  domainAuthorityRules: [
    {
      domain: 'modelcontextprotocol.io',
      pathPrefix: '/specification',
      pathContains: 'changelog',
      authority: 'official_changelog',
  ...
  repoAuthorityRules: [
    {
      ...
      repoPattern: '*',
      authority: 'official_repo',
    },
  ],

  vendorSdkDomains: [
    { domain: 'sdk.vercel.ai', authority: 'vendor_sdk_docs' },
    { domain: 'vercel.com', pathContains: '/docs', authority: 'vendor_sdk_docs' },
    { domain: '*.microsoft.com', authority: 'vendor_sdk_docs' },
    { domain: 'developers.cloudflare.com', authority: 'vendor_sdk_docs' },
  ],
  ...
  knownPackages: {
    '@ai-sdk/mcp': {
      ...
      owner: 'Vercel',
      ecosystem: 'AI SDK',
      projectCanonicalName: 'Model Context Protocol',
      entityType: 'package',
    },
  },
  ...
  sourceTypeRules: [
    ...
    { domain: 'blog.modelcontextprotocol.io', sourceType: 'official_blog' },
    ...
    { domain: 'anthropic.com', sourceType: 'vendor_docs' },
    { domain: 'vercel.com', sourceType: 'vendor_docs' },
    { domain: 'sdk.vercel.ai', sourceType: 'vendor_docs' },
  ],
...
// ── Helpers (pass ctx through) ──────────────────────────────────────────────

function source(
  overrides: Partial<SourceEntry> & Pick<SourceEntry, 'id' | 'title' | 'url'>,
): SourceEntry {
  const domain = new URL(overrides.url).hostname.replace(/^www\./, '');
  const sourceType = inferSourceTypeFromUrl(overrides.url, overrides.sourceType ?? 'web', mcpCtx);
  const base: SourceEntry = {
    ...
    sourceType,
    domain,
    authorityClass: classifySourceAuthority({ url: overrides.url, domain, sourceType }, mcpCtx),
    isPrimary: false,
...
function finding(
  overrides: Partial<Finding> & Pick<Finding, 'id' | 'claim' | 'sourceIds'>,
): Finding {
  const base: Finding = {
    id: overrides.id,
    claim: overrides.claim,
    normalizedClaim: overrides.claim
      .toLowerCase()
      .replace(/[^\w\s]/g, '')
      .trim(),
    subQuestionIds: ['sq1'],
...
test('package release must not become protocol release', () => {
  ...
  const validated = applyReportValidation(report(findings[0]!.claim), sources, findings, mcpCtx);
...
test('protocol release claims require primary official support', () => {
  ...
  const ledger = buildClaimLedger(findings, sources, 'latest MCP protocol release', mcpCtx);
...
test('publication date is not treated as release date', () => {
  ...
  const ledger = buildClaimLedger(findings, sources, 'latest MCP developments', mcpCtx);

  assert.ok(ledger[0]!.risks.includes('temporal_misattribution'));
});

test('latest official version is detected from official spec pages', () => {
  const sources = [
    source({
      id: 'spec-2025-06-18',
  ...
  const latest = detectLatestOfficialVersion(sources, 'latest MCP developments as of 2026', mcpCtx);

  assert.strictEqual(latest?.version, '2025-11-25');
  assert.strictEqual(latest?.sourceId, 'spec-2025-11-25');
});

test('source authority ranking prefers official sources over third-party commentary', () => {
...
test('source and finding counts are derived from canonical registry', () => {
  ...
  assert.strictEqual(
    validated.evidenceSources.length,
    validated.sourceRegistry?.filter((s) => s.usedInReport).length,
  );
});

test('marketing language is flagged rather than laundered as factual evidence', () => {
  const sources = [
    source({ id: 's1', title: 'Commentary', url: 'https://example.com/mcp-commentary' }),
  ];
  const findings = [
    finding({
      id: 'marketing',
  ...
  const audit = validateResearchReport(report(findings[0]!.claim), sources, findings, mcpCtx);
...
test('arbitrary docs and GitHub repos are not classified as official authority', () => {
...
test('unsafe narrative-only protocol claim is removed and audit reruns', () => {
  ...
  const validated = applyReportValidation(report(claim), sources, [], mcpCtx);
...
test('claim evidence alignment tracks missing package and version anchors', () => {
  ...
  const ledger = buildClaimLedger([f], sources, 'latest MCP package release', mcpCtx);
...
test('finding linkage clusters vector-neighbour records without all-pairs LLM calls', () => {
  const f1 = finding({
    id: 'f1',
    claim: 'LLM-CER clusters records directly in context.',
    sourceIds: ['s1'],
  });
  const f2 = finding({
    id: 'f2',
    claim: 'In-context clustering groups entity records directly.',
    sourceIds: ['s2'],
  });
  const f3 = finding({
    id: 'f3',
    claim: 'Blocking filters candidate pairs before matching.',
    sourceIds: ['s3'],
  });
  ...
  assert.ok(
    linkage.edges.some((edge) => edge.relation === 'near_duplicate' && edge.strength === 'strong'),
  );
});

test('weak semantic bridge edges do not transitively over-merge distinct entity claims', () => {
  const protocol = finding({
    id: 'protocol',
    claim: 'MCP added OAuth support for protected resources.',
    sourceIds: ['s1'],
  });
  const desktop = finding({
    id: 'desktop',
    claim: 'Claude Desktop added OAuth support for MCP servers.',
    sourceIds: ['s2'],
  });
  ...
  const protocolCluster = linkage.clusters.find((cluster) =>
    cluster.findingIds.includes('protocol'),
  );
  const bridgeEdge = linkage.edges.find(
    (edge) => edge.leftFindingId === 'protocol' && edge.rightFindingId === 'desktop',
  );
  ...
  const sources = [
    ...
    source({
      ...
      url: 'https://example.com/claude-desktop-oauth',
    }),
  ];
  const validated = applyReportValidation(
    {
      ...report(protocol.claim),
  ...
  const clusterIssue = validated.reportAudit?.issues.find(
    (issue) => issue.type === 'cluster_integrity',
  );
...
test('LLM synthesis performs an audit-guided revision pass', async () => {
  ...
  const revised = await synthesizer.synthesize(
    makeState('latest MCP protocol release', sources, findings),
  );
...
test('validated reports expose an auditable evidence graph and cluster enforcement', () => {
  const sources = [
    source({ id: 's1', title: 'MCP commentary', url: 'https://example.com/mcp-oauth' }),
  ];
  const f = finding({
    id: 'f1',
    claim: 'MCP added OAuth support for protected resources.',
  ...
  assert.ok(
    validated.evidenceGraph?.edges.some((edge) => edge.relation === 'source_provides_evidence'),
  );
  assert.ok(
    validated.evidenceGraph?.edges.some((edge) => edge.relation === 'evidence_supports_finding'),
  );
