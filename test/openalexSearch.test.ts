...
test('searchOpenAlex respects limit parameter', async () => {
  const results = await searchOpenAlex('test', 2);
  assert.ok(results.length <= 2, `expected <= 2 results, got ${String(results.length)}`);
});

test('searchOpenAlex handles empty query gracefully', async () => {
  const results = await searchOpenAlex('', 5);
  assert.ok(Array.isArray(results), 'should return an array');
});
